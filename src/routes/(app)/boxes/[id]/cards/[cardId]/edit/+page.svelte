<script lang="ts">
  ...
  let front = $state(data.card?.front ?? '');
  let back = $state(data.card?.back ?? '');
...
<div class="card-edit">
  ...
  <div class="card-edit__form">
    ...
    <PillButton
      variant={showDeleteConfirm ? 'danger-fill' : 'danger-outline'}
...
<style>
  ...
  .card-edit__error { font-size: var(--font-size-sm); color: var(--color-danger); margin: 0; }
</style>
