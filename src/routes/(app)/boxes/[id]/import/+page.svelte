<script lang="ts">
  ...
  const cards = $derived(
    firstRowIsHeader ? allRows.slice(1) : allRows
  );

  const validCards = $derived(
    cards.filter(c => c.front.length > 0 && c.back.length > 0)
  );
...
<div class="import-page">
  <TopBar
    showBack
    title={$t('box.import')}
    onback={() => goto(`/boxes/${boxId}/settings`)}
  />
  ...
  <div class="import-page__content">
    ...
    {#if !fileContent}
      ...
      >
        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="drop-zone__icon">
            ...
            <tbody>
              {#each allRows as row, i (i)}
                ...
                >
                  ...
                  <td class="preview-table__cell preview-table__cell--front">{row.front}</td>
                  <td class="preview-table__cell preview-table__cell--back">{row.back}</td>
                </tr>
              {/each}
            </tbody>
          </table>
